...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    #(2) embed_word(N,T,wordvec_dim ) captions_in(N,T,V)
    ...
    #(4) h with dim(N,T,H) , W_v with dim (h,V),b_vocab with dim (V,)
    # affine_forward_out with dim (N,T,V)
    affine_forward_out, affine_forward_cache = temporal_affine_forward(h, W_vocab, b_vocab)

    #(5) dscore with dim (N,T,V) 
