...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  #x : (N,D) ; prev_h : (N,H) ;Wx : (D,H) ;
  #Wh: (H,H) ; affine_output : (H,); next_h :(N,H)
  (x, prev_h, Wx, Wh, affine_output, next_h) = cache

  #daff size #N*H
  daffine_output = dnext_h*(1-next_h*next_h) 
  #dWx size D*H
  dWx=((daffine_output.T).dot(x)).T
  #dWh size H*H
  dWh=((daffine_output.T).dot(prev_h)).T
  #dprev_h size(N,H)
  dprev_h=(Wh.dot(daffine_output.T)).T
  #dx size(N,D)
  dx=(Wx.dot(daffine_output.T)).T
  #db size (H,)
